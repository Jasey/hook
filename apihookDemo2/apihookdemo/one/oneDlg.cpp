...
ConeDlg::ConeDlg(CWnd* pParent): CDialog(ConeDlg::IDD, pParent)
{
	...
	hAddDll=::LoadLibrary(_TEXT("Debug//add.dll"));
...
BEGIN_MESSAGE_MAP(ConeDlg, CDialog)
	...
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_DO_HOOK, &ConeDlg::OnBnClickedDoHook)
	ON_BN_CLICKED(IDC_ADD, &ConeDlg::OnBnClickedAdd)
	ON_BN_CLICKED(IDC_UNINSTALL, &ConeDlg::OnBnClickedUninstall)
END_MESSAGE_MAP()
...
//API注入要做的事情就是远程线程调用如下动作
void ConeDlg::OnBnClickedDoHook()
{
	AfxMessageBox(_T("ready load hook.dll"));
	hinst=LoadLibrary(_T("hook.dll"));
	AfxMessageBox(_T("end load hook.dll"));
	if(hinst==NULL)
	{
	...
	AfxMessageBox(_T("ready execute installhook function"));
	BOOL ret=insthook();
	AfxMessageBox(_T("end execute installhook function"));
	**/
}

void ConeDlg::OnBnClickedAdd()
{
...
void ConeDlg::OnBnClickedUninstall()
{
	...
	if(UninstallHook!=NULL) 
		...
		AfxMessageBox(_T("hook not installed"));
	}
	if (hinst!=NULL)
	{
		::FreeLibrary(hinst);
